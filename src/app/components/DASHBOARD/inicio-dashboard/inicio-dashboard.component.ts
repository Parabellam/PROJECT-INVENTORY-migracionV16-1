              ...
              .subscribe({
                next: (data) => {
                  if (this.clienteSeleccionado == 'Nexos') {
                    // Filtra todos los equipos para solo hacer conteo de los código 03
                    const equiposFiltrados = data.filter(
                      (equipo) => equipo.tipo === 'Equipos'
                    );
                    this.propiosX = equiposFiltrados.length;
                  } else {
      ...
      this.equipoService
        ...
        .subscribe({
          next: (data) => {
            if (this.clienteSeleccionado == 'Nexos') {
              // Filtra todos los equipos para solo hacer conteo de los código 03
              const equiposFiltrados2 = data.filter(
                (equipo) => equipo.tipo === 'Equipos' || equipo.tipo === 'CPU'
              );
              const equiposFiltrados3 = equiposFiltrados2.filter((f: any) => {
                return f.estado === 'Disponible';
              });
              this.p_disponibles = equiposFiltrados3.length;

              const equiposFiltrados4 = data.filter(
                (equipo) => equipo.tipo === 'Equipos' || equipo.tipo === 'CPU'
              );
              const equiposFiltrados5 = equiposFiltrados4.filter((f: any) => {
                return f.estado === 'Asignado';
              });
              this.p_asignados = equiposFiltrados5.length;
            } else {
            ...
            this.equipoService
              ...
              .subscribe({
                next: (data) => {
                  const baja = data.filter(
                    (deBaja) =>
                      deBaja.tipo === 'Equipos' || deBaja.tipo === 'CPU'
                  );
                  this.p_deBaja = baja.length;
                  resolve();
                },
                error: (err) => {
                  console.log(err);
